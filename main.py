...
def get_args_parser():
    ...
    parser.add_argument('--loss_type', default='ce', type=str, metavar='LOSS_TYPE',
                        help='Loss (default: "ce"')
    parser.add_argument('--triplet', type=bool, default=False, help='Triplet loss')
    ...
    parser.add_argument('--data-path', default='/datasets01/imagenet_full_size/061417/', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='SVRT', choices=['SVRT', 'Herbarium', 'Herbarium22', \
...
def main(args):
    ...
    if args.data_set == 'Herbarium22hier':
        dataset_train, args.nb_classes, args.family, args.genus, args.species = \
            build_dataset(is_train='train', args=args)
    else:
        dataset_train, args.nb_classes = build_dataset(is_train='train', args=args)
    dataset_val, _ = build_dataset(is_train='val', args=args)[:2]
    ...
    if not args.resume and args.ckpt:
        print(f'loading checkpoint weight from {args.ckpt}')
        model = load_state_dict(model, args.ckpt, args.nb_classes)
